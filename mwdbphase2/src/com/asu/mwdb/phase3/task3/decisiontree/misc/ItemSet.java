...
public class ItemSet {
	...
	protected Attribute entropyAttribute; 	/*  The attribute used to compute the entropy  */

	/*
	 * Used to sort numerical test values while remembering the corresponding
	 * goal value.
	 */
	private class TestGoalValues implements Comparable {
	 ...
	 * 
	 * @param attributeSet
	 *            The set of attributes describing the items of the set.
	 **/
	...
	/**
	 * Adds all the items of a vector. All the elements of the vector must be
	 * {@link Item items} compatible with this set's attribute set.
	 * 
	 * @param items
	 *            A Vector of {@link Item Items}.
	 **/
	 ...
	 * 
	 * @param index
	 *            The item index. <code>index</code> is such that 0 <=
	 *            <code>index</code> < <code>size()</code>.
	 * @param attribute
	 *            The attribute whose value is to be retreived.
	...
	/**
	 * Returns an item of the array given its index. The index is such that 0 <=
	 * <code>index</code> < <code>size()</code>.
	 * 
	 * @param index
	 *            The item index.
	...
	/**
	 * Returns the sum of this set's items weights. (This can be see as the size
	 * of this set if items are partially counted according to their weight.)
	 * 
	 ...
	 *            The set of attributes defining which attributes can be tested.
	 * @param goalAttribute
	 *            The attribute to guess using the test.
	...
	public TestScore bestSplitTest(AttributeSet candidateAttributes,
		...
		TestScore[] scores = bestSplitTests(candidateAttributes, goalAttribute);
	...
	public TestScore[] bestSplitTests(AttributeSet candidateAttributes,
		...
		for (int i = 0; i < attributes.size(); i++)
			bestTestScores[i] = bestSplitTest(
					(Attribute) attributes.elementAt(i), goalAttribute);
	...
	/**
	 * Finds the best split test. This test involves one attribute only, and
	 * aims at increasing the information on a so-called 'goal' attribute as
	 * much as possible.
	 * 
	 * @param testAttribute
	 *            The attribute on which the test is based.
	 * @param goalAttribute
	 *            The attribute to model.
	...
	private TestScore bestSplitTest(SymbolicAttribute testAttribute,
		 ...
		 * their 'goal' attribute. frequencyMatchSum[tvi] is the sum of the
		 * frequencyMatch[tvi][gvi] elements (for all gvi).
		 */
		...
		for (int i = 0; i < items.size(); i++) {
			int testValue = ((KnownSymbolicValue) item(i).valueOf(testIndex)).intValue;
			int goalValue = ((KnownSymbolicValue) item(i).valueOf(goalIndex)).intValue;
		...
		for (int tvi = 0; tvi < testNbValues; tvi++) {
			double score = entropy(goalAttribute)
					- ((frequencyMatchSum[tvi] / items.size()) * Entropy
							.entropy(frequencyMatch[tvi]))
					- ((frequencyNoMatchSum[tvi] / items.size()) * Entropy
							.entropy(frequencyNoMatch[tvi]));
		...
		for (int gvi = 0; gvi < goalNbValues; gvi++)
			remainingFrequencyNoMatch[gvi] = frequencyMatch[0][gvi]
					+ frequencyNoMatch[0][gvi];
		...
		while (remainingTestValueIndexes.size() >= 2) {
			/*
			 * Update remainingFrequency... arrays according to the last test
			 * attribute value removed
			 */
			remainingTestValueIndexes.remove(new Integer(bestValue));
			for (int gvi = 0; gvi < goalNbValues; gvi++) {
				remainingFrequencyMatch[gvi] += frequencyMatch[bestValue][gvi];
				remainingFrequencyNoMatch[gvi] -= frequencyMatch[bestValue][gvi];
			}
			remainingFrequencyMatchSum += frequencyMatchSum[bestValue];
			remainingFrequencyNoMatchSum -= frequencyMatchSum[bestValue];
			...
			for (int i = 0; i < remainingTestValueIndexes.size(); i++) {
				...
				thisFrequencyMatchSum = frequencyMatchSum[tvi]
						+ remainingFrequencyMatchSum;
				thisFrequencyNoMatchSum = remainingFrequencyNoMatchSum
						- frequencyMatchSum[tvi];

				double score = entropy(goalAttribute)
						- ((thisFrequencyMatchSum / items.size()) * Entropy
								.entropy(thisFrequencyMatch))
						- ((thisFrequencyNoMatchSum / items.size()) * Entropy
								.entropy(thisFrequencyNoMatch));
		...
		return new TestScore(new SymbolicTest(testAttribute, testValueIndexes),
				bestScore);
	}
	...
	private TestScore bestSplitTest(NumericalAttribute testAttribute,
		...
		for (int gvi = 0; gvi < goalNbValues; gvi++) {
			SymbolicTest valueTest = new SymbolicTest(goalAttribute,
		...
		for (int i = 0; i < items.size(); i++) {
			...
			tgv[i] = new TestGoalValues(
					((KnownNumericalValue) item.valueOf(testIndex)).doubleValue,
	 ...
	 * 
	 * @param test
	 *            A test on an attribute of this set's items.
	 ...
	 * 
	 * @param attribute
	 *            The attribute agains which to compute the entropy.
	...
	public double entropy(SymbolicAttribute attribute) {
		...
		if (entropy < 0. || !entropyAttribute.equals(attribute)) {
			...
			for (int i = 0; i < items.size(); i++) {
				KnownSymbolicValue sv = (KnownSymbolicValue) item(i).valueOf(
						attributeSet.indexOf(attribute));
