...
public class DriverMain {
	...
	/**
	 * Task 1: Read a gesture file and normalize it.
	 * After that assign letters to normalized files and
	 * construct dictionary out of this folder
	 * @throws IOException
	 * @throws MatlabInvocationException
	 */
	private static void indexFiles(double[][] rBandValueRange, String databaseDirectory) throws IOException,
		...
		if(!allFiles.mkdir()) {
			System.out.println("Error while creating intermediate directory, task not complete");
			return;
		}
	...
	private static void buildDictionary(File folder, double[][] rBandValueRange ) throws IOException, MatlabInvocationException {
		// Normalize data between -1 and 1
		...
		dictionary.createDictionary(wordLength, shiftLength,folder.getAbsolutePath());
	...
	private static void copyAllFiles(File source,File dest) throws IOException{
		...
		for (File file : fileNames){
			String newFileName = dest + File.separator + source.getName()+"_"+file.getName();
			File newFileObj = new File(newFileName);
			FileUtils.copyFile(file, newFileObj);
		}
	}
	private static void cleanTask1Data(String inputDirectory) throws IOException {
		File fileObj = new File(inputDirectory);
		File[] files = 	fileObj.listFiles();
		for(File file : files) {
			String fileName = file.getName();
			if(fileName.contains("gaussian") || fileName.contains("normalized") || fileName.contains("letters") 
			|| fileName.contains("task1Output") || fileName.contains("task1OutputAll") 
			|| fileName.contains("task3Output") || fileName.contains("task3OutputAll")
	...
	/**
	 * Task 3: Given a query file - find 10 most similar gesture files
	 * based on TF/IDF/TD-IDF etc values. 
	 * The user will specify parameter for comparison.
	 * @throws IOException
	 * @throws MatlabInvocationException
	 */
	private static void executeTask1b(double[][] rBandValueRange,String inputDirectory) throws IOException,
			MatlabInvocationException {
		...
		AssignBandValues.assignGaussianCurveTask3(proxy, inputFileLocation, rBandValueRange);
		DictionaryBuilderPhase2 dictionary = dictMap.get(inputDirectory);
		SearchDatabaseForSimilarity task3FindSimilarData =  new SearchDatabaseForSimilarity(dictionary.getTfIDFMapGlobal(),dictionary.getTfMapArrayIDF(),
				dictionary.getTfMapArrayIDF2(),wordLength,shiftLength, inputFileLocation,dictionary);
	 ...
	 * @throws MatlabInvocationException
	 */
	private static void executeTask1c(double[][] rBandValueRange, String databaseDirectory) throws IOException,
			MatlabInvocationException {
		...
		AssignBandValues.assignGaussianCurveTask3(proxy, inputFileLocation, rBandValueRange);
		DictionaryBuilderPhase2 dictionary = dictMap.get(databaseDirectory + File.separator + "all");
		SearchDatabaseForSimilarity task3FindSimilarData =  new SearchDatabaseForSimilarity(dictionary.getTfIDFMapGlobal(),dictionary.getTfMapArrayIDF(),
				dictionary.getTfMapArrayIDF2(),wordLength,shiftLength, inputFileLocation,dictionary);
