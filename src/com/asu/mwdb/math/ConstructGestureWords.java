...
//		constructGestureWordsW.constructGestureWords(w, s, sampleDataLoc+"\\letter\\W", sampleDataLoc+"\\task1\\W",sampleDataLoc+"\\task2\\W");
...
//				BufferedWriter bufferWriter = new BufferedWriter(fileWriter);
				  while (it.hasNext()) {
				       ...
				       // Double idf = pairs.getValue().get(0)*(Math.log(inverse));
				        Double idf = (Math.log(inverse)); //IDF Change by akshay
